DELIMITER $$
CREATE procedure monthly_billing_proc()
BEGIN
  ...
  owner_loop: LOOP
   ...
   SET total_rent = (SELECT rent FROM apartment WHERE owner_cnic = owner_cnic);
    ...
    SET total_food_payment =  (SELECT SUM(o.amount)  FROM foodOrder AS fo
    INNER JOIN orderContains AS oc ON fo.order_id = oc.order_id
    WHERE fo.owner_cnic = owner_cnic AND fo.date_placed BETWEEN
        CONCAT(year, '-', MONTH(CURDATE() - INTERVAL 1 MONTH), '-01') AND
        LAST_DAY(CONCAT(year, '-', MONTH(CURDATE() - INTERVAL 1 MONTH), '-01')));
